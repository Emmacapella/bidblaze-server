...
// --- CHAT PERSISTENCE ---
let chatHistory = [];
// Load last 50 messages from DB on server start
async function loadChat() {
    ...
    } catch (e) { console.log("Chat DB not initialized yet"); }
}
loadChat();
...
async function loadGameStates() {
    ...
    } catch (e) { console.error("Failed to load game states:", e); }
}
...
async function executeBid(email, roomKey, isAutoBid = false) {
    ...
    if (isAutoBid) {
        displayUser = BOT_ALIASES[Math.floor(Math.random() * BOT_ALIASES.length)];
    }
